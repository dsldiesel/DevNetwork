...
const AddExperience = ({ addExperience, history }) => {
  ...
  const onSubmit = e => {
    e.preventDefault();
    addExperience(formData, history); // history will allow to redirect in action file
  };
  const { title, company, location, from, to, current, description } = formData; // So we can use them separately
  return (
    <Fragment>
...
export default connect(
  null,
  { addExperience }
)(withRouter(AddExperience)); // Wraps the component so we can pass the history object!
